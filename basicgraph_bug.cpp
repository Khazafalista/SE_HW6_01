...
/*
	Review: the variable keyInput is already declared
*/
int keyInput = 0;
...
void findMinMax(GLfloat &xMax, GLfloat &xMin, GLfloat &yMax, GLfloat &yMin, GLfloat &zMax, GLfloat &zMin) {
	for (int i = 0; i != myVertices.size(); i+=3) {
		...
		if (xMin > vx)xMin = vx;
		/*
			Review: yMax and yMin cases are not considered
		*/
		if (zMax < vz)zMax = vz;
		if (zMin > vz)zMin = vz;
	}
...
void translateToMiddle(GLfloat xCenter, GLfloat yCenter, GLfloat zCenter) {
	/*
		Review: transforming x(y, z)Center to x(y, z)Trans and then transforming back is unnecessary
	*/
	GLfloat xTrans = -xCenter, yTrans = -yCenter, zTrans = -zCenter;
	for (int i = 0; i != myVertices.size(); i+=3)
		myVertices[i] += xTrans;
...
void traverseColorModel()
{
	...
	GLfloat xMax = FLT_MIN, yMax = FLT_MIN, zMax = FLT_MIN, xMin = FLT_MAX, yMin = FLT_MAX, zMin = FLT_MAX;
	/*
		Review: x(y, z)Center used before declaration. Also, the condition that vertex value after the operation less than FLT_MIN is not considered
	*/
	translateToMiddle(xCenter, yCenter, zCenter);
	//cout << "xMax=" << xMax << ", xMin=" << xMin << ", yMax=" << yMax << ", yMin=" << yMin << ", zMax=" << zMax << ", zMin=" << zMin << endl;
	GLfloat xCenter = (xMax + xMin) / 2, yCenter == (yMax + yMin) / 2, zCenter = (zMax + zMin) / 2;
...
int loadOBJModel()
{
	...
	OBJ = glmReadOBJ(filename);
	/*
		Review: read object result unchecked
	*/
	printf("%s\n", filename);
...
void showShaderCompileStatus(GLuint shader, GLint shaderCompiled)
{
	glGetShaderiv(shader, GL_COMPILE_STATUS, shaderCompiled);
    /*
        Review: should be GL_FALSE == (*shaderCompiled)
    */
	if(GL_FALSE = (*shaderCompiled))
	{
        /*
...
void onKeyboard(unsigned char key, int x, int y) 
{
	...
	switch(key) 
	{
		...
		case 0x7a://z
			--modelId;
            /*
                the operation || 1 would make it always true
            */
			if (modelId == -1 || 1) {
				modelId = 3;
			}
		...
		case 0x77:
			...
			break;
        /*
            Review: the case 0x78 is already considered
        */
		case 0x78://x
			++modelId %= 4;
			changeModel(modelId);
